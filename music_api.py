...
import json
#print api.search(u'好想你')
#t=json.loads(api.search(u'好想你'))
t=api.search(u'好想你')
mId=t['result']['songs'][0]['id']
print mId
murl=api.song_detail(mId)
murl=murl['songs'][0]['mp3Url']
print murl

#http://m2.music.126.net/9O4zqYcb8w-ahv98Zbkg_A==/3253454913024738.mp3
import socket
#!/usr/bin/python
import socket
HOST="223.6.248.220"#'www.cnblogs.com'
PORT=80
'''
try:
    s=socket.socket(socket.AF_INET,socket.SOCK_STREAM)      #定义socket类型，网络通信，TCP
    s.connect((HOST,PORT))       #要连接的IP与端口
    s.send("GET "+"/ranyonsue/p/5984001.html"+" HTTP/1.1\r\n")
    s.send('Host:' +HOST+'\r\n')
    s.send("Connection: Close\r\n")
    #s.send("Accept-Encoding: mp3,gzip, deflate, sdch\r\n")
    s.send("Accept-Language: zh-CN,zh;q=0.8\r\n")
    #s.send("User-Agent: Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/49.0.2623.110 Safari/537.36\r\n")

    #s.send("Cache-Control: max-age=0\r\n")
    s.send("Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8")
    s.send("Accept-Encoding:gzip, deflate, sdch")
    s.send("\r\n")
...
headers = {"Content-type": "application/x-www-form-urlencoded",
               "Accept": "*/*",
           "Connection": "keep-alive"}
conn=httplib.HTTPConnection("m2.music.126.net")
conn.request('GET','/9O4zqYcb8w-ahv98Zbkg_A==/3253454913024738.mp3',headers=headers)
